...
class ModRedshopMegaMenuHelper
{
    ...
    public static function getCategories($params)
    {
        ...
        foreach ($categories as $category) {
            ...
            $categoryMenuItem = $menu->getItems(
                'link',
                'index.php?option=com_redshop&view=category&layout=detail&cid=' . $category->id . '&manufacturer_id=0',
                true
            );

            $category->name  = str_replace('- ', '', $category->name);
            $category->link  = JRoute::_(
        ...
        foreach ($firstItem as $subCategory) {
            ...
            if ($subCategory->category_id != 0) {
                $subCategory->sub_cat = self::getListForRedshopMegamenu(
                    $categories,
    ...
    public static function getListForRedshopMegamenu($items, $parentId, $level, $end)
    {
        ...
        foreach ($items as $item) {
            if ($item->parent_id == $parentId) {
                ...
                $categoryMenuItem = $menu->getItems(
                    'link',
                    'index.php?option=com_redshop&view=category&layout=detail&cid=' . $item->id . '&manufacturer_id=0',
                    true
                );

                $subItem[$key]->category_name = str_replace('- ', '', $item->name);
                $subItem[$key]->category_id   = $item->id;
                $subItem[$key]->link          = JRoute::_(
                    'index.php?option=com_redshop&view=category&layout=detail&cid='
            ...
            if ($level == 1) {
                ...
                if (!empty($items[$i]->image)) {
                    echo '<img src="' . JUri::root(
                        ) . 'components/com_redshop/assets/images/category/' . $items[$i]->image . '" />';
                }
     ...
     */
    public
    static function getLinkAttributes(
        $attr
    ) {
        return implode(
            ' ',
     ...
     */
    public
    static function sortCategories(
        &$categories = [],
        $sortBy = 'name',
        $sortDestination = 'asc'
    ) {
