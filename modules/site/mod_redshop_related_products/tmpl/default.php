...
<div class="mod_redshop_products_wrapper" id="<?php echo $moduleId ?>">
	<div id="<?php echo $moduleId ?>_products_wrapper">
		<?php foreach ($rows as $row): ?>
				...
				if (!$isStockExists) {
					...
					if (($productPreorder == "global" && \Redshop::getConfig()->get(
						...
						if (!$isPreorderStockExists) {
							$stockStatus = "<div class=\"modProductStockStatus mod_product_outstock\"><span></span>" . JText::_(
									'COM_REDSHOP_OUT_OF_STOCK'
								) . "</div>";
						} else {
							$stockStatus = "<div class=\"modProductStockStatus mod_product_preorder\"><span></span>" . JText::_(
					...
					$stockStatus = "<div class=\"modProductStockStatus mod_product_instock\"><span></span>" . JText::_(
			...
			$link         = JRoute::_(
				'index.php?option=com_redshop&view=product&pid=' . $row->product_id . '&cid=' . $categoryId . '&Itemid=' . $itemId
			);
			...
			<div class="<?php echo $wrapperClass ?>">
				...
				if (!$row->not_for_sale && $showPrice) {
					...
					if (\Redshop::getConfig()->getBool('SHOW_PRICE') && (!\Redshop::getConfig()->get(
							...
							if ($productOldPrice > $productPriceDiscount) {
								...
								<?php if ($showDiscountPriceLayout): ?>
									...
									<div id="mod_redoldprice"
									     class="mod_redoldprice"><?php echo \RedshopHelperProductPrice::formattedPrice(
											$productOldPrice
										) ?></div>
									<div id="mod_redmainprice"
									     class="mod_redmainprice"><?php echo \RedshopHelperProductPrice::formattedPrice(
											$productPriceDiscount
										) ?></div>
									<div id="mod_redsavedprice"
									     class="mod_redsavedprice"><?php echo JText::_(
												'COM_REDSHOP_PRODCUT_PRICE_YOU_SAVED'
											) . ' ' . \RedshopHelperProductPrice::formattedPrice($savingPrice) ?></div>
								<?php else: ?>
									<?php $productPrice = $productPriceDiscount; ?>
									<div class="mod_redshop_products_price"><?php echo \RedshopHelperProductPrice::formattedPrice(
											$productPrice
										) ?></div>
								<?php endif; ?>
								<?php
				...
				?>
				<?php if ($showWishlist): ?>
					<div class="wishlist"><?php echo \RedshopHelperWishlist::replaceWishlistTag(
							$row->product_id,
							'{wishlist_link}'
						) ?></div>
				<?php endif; ?>
				...
				<?php if (isset($showAddToCart) && $showAddToCart): ?>
					...
					if ($row->attribute_set_id > 0) {
						$setOfAttributes = \Redshop\Product\Attribute::getProductAttribute(
							0,
					...
					if (\Redshop::getConfig()->getBool('AJAX_CART_BOX')) {
						...
						if (isset($ajaxDetailTemplate->template_desc) && !empty(
							trim(
								$ajaxDetailTemplate->template_desc
							)
							)) {
							$ajaxDetailTemplateDesc = $ajaxDetailTemplate->template_desc;
						}

						$returns           = \Redshop\Product\Product::getProductUserfieldFromTemplate(
							$ajaxDetailTemplateDesc
						);
						...
						if (!empty($templateUserfield)) {
							...
							foreach ($userFields as $item) {
								$productUserfields = \RedshopHelperExtrafields::listAllUserFields(
									$item,
									12,
									'',
									'',
