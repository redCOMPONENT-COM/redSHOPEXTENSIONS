 ...
 * @subpackage  Plg\Redshop_PaymentRs_Payment_BankTransfer_Discount
 *
...
/**
 *  Plg\Redshop_PaymentRs_Payment_BankTransfer_Discount installer class.
 *
 * @package  \Redshopb.Plugin
 * @since    1.7.0
 */
class Plg\Redshop_PaymentRs_Payment_BankTransfer_DiscountInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->q('rs_payment_banktransfer_discount'))
