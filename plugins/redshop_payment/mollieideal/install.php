...
class PlgRedshop_PaymentMollieidealInstallerScript
{
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('mollieideal'))
			)
				->loadResult();
	...
	protected function deleteOldLanguages()
	{
		...
		// Remove old library
		$oldFolder = JPATH_ROOT . '/plugins/redshop_payment/mollieideal/library/src';
		...
		// Remove old file
		$oldFile = JPATH_ROOT . '/plugins/redshop_payment/mollieideal/library/initialize.php';
