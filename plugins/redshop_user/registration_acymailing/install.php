 ...
 * @subpackage  Plg\Redshop_UserRegistration_Acymailing
 *
...
/**
 *  Plg\Redshop_UserRegistration_Acymailing installer class.
 *
 * @package  \Redshopb.Plugin
 * @since    1.7.0
 */
class Plg\Redshop_UserRegistration_AcymailingInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->q('registration_acymailing'))
					->where($db->qn('folder')) . ' = ' . $db->q('redshop_user')
			)->loadResult();
