...
/**
 * Plg\Redshop_ProductShoppergroup_Tags installer class.
 *
 * @package  \Redshopb.Plugin
 * @since    1.6.0
 */
class Plg\Redshop_ProductShoppergroup_TagsInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('folder') . ' = ' . $db->quote('redshop_product'))
					->where($db->qn('element') . ' = ' . $db->quote('shoppergroup_tags'))
			)
				->loadResult();
